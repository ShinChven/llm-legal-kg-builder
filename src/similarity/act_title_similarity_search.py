...
def find_similar_acts(title: str, threshold: float = 0.99, limit: int = 10):
    """
    Finds similar acts based on title embedding similarity using pgvector.

    Args:
        title: The title of the act to search for.
    ...
    query_embeddings = get_embeddings([title])
    if not query_embeddings:
        print(f"Could not generate embedding for title: {title}")
        return []
    query_embedding = query_embeddings[title]
