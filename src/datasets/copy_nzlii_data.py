...
class ExtraDBConnection:
    """Database connection for the extra database containing nzlii data"""
...
def fetch_nzlii_data(extra_db_conn) -> List[Dict[str, Any]]:
    """Fetch all nzlii data from the extra database"""
    ...
    try:
        ...
        print(f"Found {len(data)} records from nzlii in extra database")
        return data

    except (Exception, psycopg2.DatabaseError) as error:
        print(f"Error fetching nzlii data: {error}")
        return []
    finally:
        cursor.close()
...
def insert_record(main_db_conn, record: Dict[str, Any]) -> bool:
    ...
    insert_query = """
        INSERT INTO legislations (
            title, source, doc_type, year, act_no, text, word_count, pdf_link, xml_link
        ) VALUES (
            %s, %s, %s, %s, %s, %s, %s, %s, %s
        )
    """
    ...
    mapped_data = (
        ...
        'nzlii.org',                       # source -> 'nzlii.org'
        'act',                             # doc_type -> 'act' (hardcoded)
        record['year'],                    # year -> year
        str(record['number_of_legislation']).zfill(4),   # number_of_legislation -> act_no (left-padded to 4 digits)
...
def copy_nzlii_data():
    """Main function to copy nzlii data from extra database to main database"""
    ...
    try:
        # Fetch nzlii data from extra database
        nzlii_records = fetch_nzlii_data(extra_conn)

        if not nzlii_records:
            print("No nzlii records found to copy")
            return

        inserted_count = 0
        skipped_count = 0
        error_count = 0
        failed_records = []  # Collect failed insertions

        # Process each record
        for record in nzlii_records:
            title = record['title']
...
if __name__ == "__main__":
    print("Starting nzlii data copy operation...")
    copy_nzlii_data()
